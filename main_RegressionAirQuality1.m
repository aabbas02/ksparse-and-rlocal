...
%[U,SY,V] = svd(Y,'econ');
%SY
%X = X + 1000000*eye(size(X,1),size(X,2));
%cond(X)
%h = 10;
%X = U(:,1:h)*S(1:h,1:h)*V(:,1:h)';
[U,S1,V] = svd(X,'econ');
%S1
%Y = U(:,1:h)*S(1:h,1:h)*V(:,1:h)';
...
%blk_label = A(:,1) + A(:,9);
%blk_label = A(:,9) + A(:,10);              % large r, proposed better
%blk_label = A(:,2) + A(:,9) +A(:,10); 
%blk_label = A(:,10);
%blk_label = A(:,9);
%blk_label = A(:,9) + A(:,1);
blk_label = A(:,1) + A(:,2);
...
% %---------------- proposed ----------------------------------
% tic 
% [pi_hat]     = lp_ls_alt_min_prox(X,Y_permuted,r_,r_local,cnvgnce);
% tProposed = toc;
...
% beta_sls = X(pi_hat,:) \ Y_permuted;
% %beta_RLUS_err = norm(beta_sls - beta_star,2)/norm(beta_star,2); 
% R2_RLUS      = 1 - norm(Y-X*beta_sls,'fro')^2/norm(Y,'fro')^2;
% tRlus = toc;
%----------------- Spectral -----------------------------------
tProposed
%tSls
