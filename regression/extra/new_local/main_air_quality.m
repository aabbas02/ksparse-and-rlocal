...
str = str(1:k(end));
cd (str)
addpath('C:\Users\aabbas02\Desktop\ULS_regression_expreiments\new_local\code') 
%------------------------------------------------------------------------------
A = readmatrix('air_quality_data.csv');
idx1 = find(A(:,2) == 2016,1,'first');
...
Y = A(:,[6,7,8,9,11]);
Y = sqrt(Y);
B = zeros(size(A,1),28-1);
B(:,1:6) = A(:,[10,12,13,14,15,16]); % change WPSM column 17 to column 16 due to ---
B(:,7:12)  = B(:,1:6).^2;
t=13;
for i = 1 : 6
    for j = i+1:6
...
n = size(Y,1);
beta_star = B \ Y;
R_2_true = 1 - norm(Y-B*beta_star,'fro')^2/norm(Y - mean(Y,1),'fro')^2
aad_true = norm(B*beta_star - Y,1)/n
...
Y_permuted = pi_map*Y;
%beta_naive = B \ Y_permuted;
%R_2_naive  = 1 - norm(Y-B*beta_naive,'fro')^2/norm(Y - mean(Y,1),'fro')^2
n = size(Y_permuted,1);
m = size(Y_permuted,2);
d = size(B,2);
frac_shuffle = (n - sum(sum(pi_map(1:size(A,1)==(1:size(A,1))'))))/(n)
%--------------------------------------------------------------------------
[pi_hat]   = lp_ls_alt_min_prox_udgp(B,Y_permuted,n,r_,1);
%d_H_pro    = sum(sum(pi_hat ~= pi_map))/(2*n)
beta_pro   = B \ (pi_hat'*Y_permuted);
R_2_pro    = 1 - norm(Y-B*beta_pro,'fro')^2/norm(Y - mean(Y,1),'fro')^2
%RMSE_pro   = sqrt(norm(beta_star - beta_pro,'fro')^2/(m*d))
%RMSE_pro_1 = sqrt(norm(beta_star(1:end-1,:) - beta_pro(1:end-1,:),'fro')^2/(m*(d-1)))
%aad_pro    = norm(B*beta_pro - pi_hat'*Y_permuted,1)/n
%--------------------------------------------------------------------------
...
%--------------------------------------------------------------------------
noise_var = norm(Y-B*beta_star,'fro')^2/(size(Y,1)*size(Y,2));
[pi_hat,beta_sls] = slawski(B,Y_permuted,noise_var,r_);
...
beta_sls    = B \ (pi_hat'*Y_permuted);
R_2_sls     = 1 - norm(Y-B*beta_sls,'fro')^2/norm(Y - mean(Y,1),'fro')^2
%RMSE_sls    = sqrt(norm(beta_star - beta_sls,'fro')^2/(m*d))
