...
%aad_true = norm(X*beta_star - Y,'fro')/norm(Y,'fro');
...
for i = 1 : length(temp)
	A(A(:,2)==temp(i),2) = i*1e2; 
end
temp = unique(A(:,9));  % month
for i = 1 : length(temp) % temperar_ture
...
aad_naive  = norm(X*beta_naive - Y,'fro')/norm(Y,'fro');
%---------------- proposed ----------------------------------
tic 
...
%aad_sls     = norm(X*beta_sls - pi_hat'*Y_permuted,'fro')/norm(Y_permuted,'fro');
%aad_sls      = norm(X*beta_sls - Y,'fro')/norm(Y,'fro');
...
R2_pro
R2_sls
%b_err_pro = norm(beta_pro - beta_star,'fro')^2/norm(beta_star,'fro')^2
%b_err_sls = norm(beta_sls - beta_star,'fro')^2/norm(beta_star,'fro')^2
